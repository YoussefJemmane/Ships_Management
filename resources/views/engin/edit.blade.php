        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <section>
                        <form method="post" action="{{ route('engin.update', ['id' => $engin->id]) }}" class="mt-6 space-y-6">
                            @csrf

                            <div>
                                <x-input-label for="code" :value="__('Code Engin')" />
                                <x-text-input id="code" name="code" type="text" class="mt-1 block w-full"
                                    value="{{ $engin->code }}" required autofocus autocomplete="code" />
                            ...
                            <div>
                                ...
                                value="{{ $engin->libelle }}" required autofocus autocomplete="libelle" />
                            ...
                            <div class="flex justify-around">
                                <div class="w-full pr-3">
                                    ...
                                    value="{{ $engin->heure_debut_affectation }}" required autofocus autocomplete="heure_debut_affectation" />
                                ...
                                <div class="w-full">
                                    ...
                                    <x-text-input id="heure_fin_affectation" name="heure_fin_affectation" type="time"
                                        class="mt-1 block w-full" min="1" max="3" value="{{ $engin->heure_fin_affectation }}" required
                                ...
                                <div class="w-full ">
                                    ...
                                    <select name="navire_id" id=""
                                        ...
                                        @foreach ($navires as $navire)
                                            <option value="{{ $navire->id }}" {{ $navire->id == $engin->navire_id ? 'selected' : '' }}>{{ $navire->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
