        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <section>
                        <form method="post" action="{{ route('arret.update', ['id' => $arret->id]) }}" class="mt-6 space-y-6">
                            @csrf

                            <div>
                                <x-input-label for="code" :value="__('Code arret')" />
                                <x-text-input id="code" name="code" type="text" class="mt-1 block w-full"
                                    value="{{ $arret->code }}" required autofocus autocomplete="code" />
                            ...
                            <div class="flex justify-around">
                                ...
                                <div class="w-full pr-3">
                                    ...
                                    <x-text-input id="heure_debut_arret" name="heure_debut_arret" type="time"
                                        class="mt-1 block w-full" min="1" max="3" value="{{ $arret->heure_debut_arret }}" required
                                ...
                                <div class="w-full">
                                    ...
                                    <x-text-input id="heure_fin_arret" name="heure_fin_arret" type="time"
                                        class="mt-1 block w-full" min="1" max="3" value="{{ $arret->heure_fin_arret }}" required
                                ...
                                <div class="w-full pr-3">
                                    ...
                                    <select name="navire_id" id="navire"
                                        ...
                                        @foreach ($navires as $navire)
                                            <option value="{{ $navire->id }}" {{ $navire->id == $arret->navire_id ? 'selected' : '' }}>{{ $navire->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="w-full ">
